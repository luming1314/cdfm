...
def EM_onestep(f_pre,I,V,HyperP,lamb=0.5,rho=0.01): 
    ...
    F = I-X # todo 这是原来的，奇怪了这个效果是正常的， 我想到一种解释方式就是 I + V - F = F
...
def EM_onestep_latent(f_pre, F_LAB, HyperP, lamb=0.5, rho=0.01):
    device = f_pre.device
...
def EM_onestep_plus(f_pre, img_list, HyperP, lamb=0.5, rho=0.01):
    device = f_pre.device
...
def EM_onestep_plus_bak(f_pre, A_img, B_img, C_img, HyperP, lamb=0.5, rho=0.01):
    device = f_pre.device
    ...
    F = A_img + B_img - X - f_pre  # todo 这是原来的，奇怪了这个效果是正常的， 我想到一种解释方式就是 I + V - F = F
