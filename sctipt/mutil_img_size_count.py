...
def count_image_sizes(folder_path):
    """
    统计文件夹中所有图片的尺寸、数量以及对应的子文件夹名称
    """
    size_counts = defaultdict(set)  # 使用set来记录子文件夹名称，避免重复
    ...
    for dirpath, dirnames, filenames in os.walk(folder_path):
        for filename in filenames:
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.gif')):
                ...
                size = get_image_size(image_path)
                parent_folder = os.path.basename(os.path.dirname(image_path))  # 获取子文件夹名称
                size_counts[size].add(parent_folder)  # 将子文件夹名称添加到对应尺寸的集合中
...
fusion_type = 'OE-UE' # IR-VI {TNO/RoadScene/M3FD/MSRS} Far-Near{Lytro/MFI-WHU} OE-UE{MEFB}
dataset_type = 'SICE'
folder_path = os.path.join(root_path, fusion_type, dataset_type) # 请替换为你的图片文件夹路径
